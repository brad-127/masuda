...
class Rollout(object):
    def __init__(self, ob_space, ac_space, nenvs, nsteps_per_seg, nsegs_per_env, nlumps, envs, policy,
        ...
        self.buf_touch = np.empty((nenvs, self.nsteps, *[2]), np.float32) # or 2
    ...
    def collect_rollout(self):
        ...
        if self.recorder is not None:
            self.recorder.dump_episode(0, self.buf_rews)
        self.update_info()
    ...
    def calculate_reward(self):
        ...
        #self.buf_rews[:] = self.reward_fun(int_rew=np.sqrt(var_rew), ext_rew=self.buf_ext_rews)
    ...
    def rollout_step(self):
        ...
        for l in range(self.nlumps):
            ...
            if infos != {}:
                ...
                #tch = infos[0].get('touch_all')
                self.buf_touch[sli,t] = tch
