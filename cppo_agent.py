...
class PpoOptimizer(object):
    ...
    def __init__(self, *, scope, ob_space, ac_space, stochpol,
        ...
        with tf.variable_scope(scope):
            ...
            self.stochpol = stochpol #policy(cnn)
            ...
            neglogpac = self.stochpol.pd.neglogp(self.stochpol.ph_ac) #negative log probability (PPO calc)
    ...
    def start_interaction(self, env_fns, dynamics_list, nlump=2):
        ...
        with tf.name_scope("summary"):
            ...
            writer=tf.summary.FileWriter(logdir + "/tb", getsess().graph)
    ...
    def update(self):
        ...
        for i in range(self.rollout.buf_obs_last.shape[0]):
            ...
            #Image.fromarray((obs).astype(np.uint8)).save('logs/images/%04d.png'%i)
            cv2.imwrite('logs/images/%04d.png'%i, (obs).astype(np.uint8))
        ...
        ph_buf = [
            ...
            (self.stochpol.ph_touch, resh(self.rollout.buf_touch)),
        ]
        ph_buf.extend([
