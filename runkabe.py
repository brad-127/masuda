...
def start_experiment(**args):
    ...
    trainer = Trainer(make_env=make_env,num_timesteps=args['num_timesteps'], hps=args,envs_per_process=args['envs_per_process'],num_dyna=args['num_dynamics'])
    ...
    # sess ...
        #print("wait for input 0")
        #ri = input(">>> ")
        #screen = gym.make('arms-v0')
        #screen = gym.make('finger-v0')
        screen = gym.make('fingerLkabe-v0')
        #screen = gym.make('armEnv-v0')
        ...
        for j in range(5):
          ...
          for i in range(10000):
            ...
            #clock.tick(60)
            ob=np.array([ob])
            a, vpred, nlp = trainer.policy.get_ac_value_nlp(ob)
            ob, reward, done, memo = screen.step(a[0])
            rew += reward
            if done:
              rew_list.append(rew)
              ep_list.append(ep)
              ...
              screen.reset()
            #if i == 10 and j == 0:
            #  pil_img = Image.fromarray(ob.shape(screen.rectSIZE[0],screen.rectSIZE[1]))
            #  pil_img = pil_img.convert("L")
            #  pil_img.save("aaa.png")

          print("ep_mean:")
          print(np.mean(ep_list))
          print("rew_mean:")
          print(np.mean(rew_list))
          rew_per_step = [rew_list[i]/ep_list[i] for i in range(len(rew_list))]
          print("rew per step:")
          print(np.mean(rew_per_step))



class Trainer(object):
    def __init__(self, make_env, hps, num_timesteps, envs_per_process, num_dyna, var_output=True):
        ...
        self.feature_extractor = {"none": FeatureExtractor,
                                  ...
                                  "pix2pix": JustPixels}[hps['feat_learning']]  #choice hps[] from {}
        ...
        for i in range(num_dyna):
            self.dynamics_list.append(self.dynamics_class(auxiliary_task=self.feature_extractor,
                                                          predict_from_pixels=hps['dyn_from_pixels'],  # it's 0
    ...
    def train(self,sess):
        ...
        while True:
            ...
            if info['update']:
                #if s_count % 1000 == 0:
                    #saver.save(sess, logdir + '/my-model', global_step=s_count)
...
def make_env_all_params(rank, add_monitor, args):
    ...
    #env = gym.make('arms-v0')
    #env = gym.make('finger-v0')
    env = gym.make('fingerLkabe-v0')
...
def get_experiment_environment(**args):
    ...
    logger_context = logger.scoped_configure(dir='./logs/' +
                                                 datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S-%f"),  # folder name is written here
...
def add_optimization_params(parser):
    ...
    parser.add_argument('--lr', type=float, default=4e-5) # default 1e-4 -> tac:4e-5 -> 1e-3
    parser.add_argument('--ent_coeff', type=float, default=0.001)
    parser.add_argument('--nepochs', type=int, default=3)
    parser.add_argument('--num_timesteps', type=int, default=int(5.001e6))   #5.001e6     #default 1e8


def add_rollout_params(parser):
    parser.add_argument('--nsteps_per_seg', type=int, default=100)
    parser.add_argument('--nsegs_per_env', type=int, default=1)
    parser.add_argument('--envs_per_process', type=int, default=1) # default 8
