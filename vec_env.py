...
class ShmemVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, spaces=None):
        ...
        if spaces:
            ...
            # space is not none, so this can ignore
    ...
    def reset(self):
        ...
        self.obs_mine = self.my_env.reset()
        return np.array([self.obs_mine])
        #return self._decode_obses([pipe.recv() for pipe in self.parent_pipes])

    def step_async(self, actions):
        assert len(actions) == len(self.parent_pipes)
        self.obs_mine, self.my_rews, self.my_dones, self.my_infos= self.my_env.step(actions[0])
        if self.my_dones:
            self.obs_mine = self.my_env.reset()
    ...
    def step_wait(self):
        ...
        return np.array([self.obs_mine]), np.array([self.my_rews]), np.array([self.my_dones]), [self.my_infos]
...
def _subproc_worker(pipe, parent_pipe, env_fn_wrapper, obs_buf, obs_shape):
    ...
    env = Monitor(env, osp.join(logger.get_dir(), '%.2i' % 1))
    ...
    try:
        while True:
            cmd, data = pipe.recv()
            if cmd == 'reset':
                print("pipe_reset start")
                my_obs = env.reset()
                print("reset_end before pipe")
                pipe.send(_write_obs(my_obs))
                print("pipe_reset end")
            elif cmd == 'step':
